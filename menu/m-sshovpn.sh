...
Pass=1
iplim=1
ExpireTime=$(date -d "+60 minutes" +"%s")  # Timestamp UNIX
if [ ! -e /etc/xray/sshx ]; then
mkdir -p /etc/xray/sshx
fi
if [ -z ${iplim} ]; then
...
# Jadwalkan penghapusan otomatis akun setelah timer selesai
echo "/usr/sbin/userdel -r $Login 2>/dev/null && sed -i '/^$Login /d' /etc/xray/ssh" | at now + $timer minutes
